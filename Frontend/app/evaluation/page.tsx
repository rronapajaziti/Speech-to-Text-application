...
export default function EvaluationPage() {
  ...
  const [transcription, setTranscription] = useState<BackendTranscription | null>(
    null,
  );
  ...
  async function recomputeWer() {
    if (!transcription?.id) {
      setMessage("Create a transcription first.");
      return;
    }
    if (!referenceText.trim()) {
      setMessage("Reference text is required.");
    ...
    setMessage("Recomputing WER...");
    try {
      const response = await fetch(
        `${apiBase}/transcriptions/update/${transcription.id}/`,
        {
          method: "PUT",
      ...
      if (!response.ok) {
        const errText = await response.text();
        throw new Error(errText || "Failed to recompute WER.");
      }
      ...
      const merged: BackendTranscription = {
        ...transcription,
        id: updated.id,
        audio_id: updated.audio,
        raw_text: updated.raw_text,
        reference_text: updated.reference_text,
        wer_score: updated.wer_score,
        model_name: updated.model_name,
        status: updated.status,
        date_created: updated.date_created,
      };
      setTranscription(merged);
      setMessage("WER updated successfully.");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Failed to recompute.");
    } finally {
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-3xl font-bold text-[#0F172A]">Audio Evaluation</h1>
          <p className="text-sm text-[#64748B]">
            Upload a file, transcribe, then evaluate with required reference text.
          </p>
        </div>
      </header>
        ...
        <div className="space-y-4 lg:col-span-5">
          <div className="rounded-2xl border border-[#E7E5E4] bg-white p-5 shadow-sm">
              ...
              <input
                ...
                onChange={async (event) => {
                  ...
                  try {
                    ...
                    setDuration(0);
                    setMessage(
                      e instanceof Error ? e.message : "Failed to read file info.",
                    );
                  }
                }}
              />
                ...
                <div className="rounded-lg border border-[#E7E5E4] bg-white p-2">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (found) setLanguageLabel(`${found.language_name} (${found.code})`);
                    }}
                    className="mt-1 w-full rounded-md border border-[#E7E5E4] bg-white px-2 py-1 text-xs font-medium text-[#0F172A] outline-none"
                  >
                    {languages.length ? (
            ...
            <div className="mt-4 space-y-2 text-sm text-[#334155]">
              ...
              <div className="flex justify-between">
                ...
                <span>
                  {transcription?.model_name || "Waiting for upload"}
                </span>
              </div>
              ...
              <div className="flex justify-between">
                ...
                <span>
                  {transcription?.status || "Not processed"}
                </span>
              </div>
          ...
          <div className="rounded-2xl border border-[#E7E5E4] bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-4xl font-semibold text-[#0F172A]">
              {transcription?.wer_score !== null && transcription?.wer_score !== undefined
                ? `${(transcription.wer_score * 100).toFixed(2)}%`
                : "N/A"}
            </p>
            <p className="mt-1 text-xs text-[#00814D]">
              {werPercent < 15 ? "Good" : werPercent < 30 ? "Fair" : "Needs work"}
            </p>
              ...
              <div className="border-t border-[#E7E5E4] p-4">
                ...
                <div className="mt-3 grid grid-cols-2 gap-3 text-sm text-[#0F172A] md:grid-cols-4">
                  <div className="rounded-lg bg-[#F5F5F4] p-2">
                    ...
                    <div className="font-semibold">{(stats.mer * 100).toFixed(2)}%</div>
                  </div>
                  <div className="rounded-lg bg-[#F5F5F4] p-2">
                    <div className="text-xs text-[#64748B]">WIP</div>
                    <div className="font-semibold">{(stats.wip * 100).toFixed(2)}%</div>
                  </div>
                  <div className="rounded-lg bg-[#F5F5F4] p-2">
                    <div className="text-xs text-[#64748B]">WIL</div>
                    <div className="font-semibold">{(stats.wil * 100).toFixed(2)}%</div>
                  </div>
                  <div className="rounded-lg bg-[#F5F5F4] p-2">
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 text-xs text-[#334155] md:grid-cols-4">
                  <div className="rounded-lg bg-[#F5F5F4] p-2">
                    ...
                    <div className="font-semibold text-[#0F172A]">{stats.substitutions}</div>
                  </div>
                  <div className="rounded-lg bg-[#F5F5F4] p-2">
                    <div className="text-[#64748B]">Del</div>
                    <div className="font-semibold text-[#0F172A]">{stats.deletions}</div>
                  </div>
                  <div className="rounded-lg bg-[#F5F5F4] p-2">
                    <div className="text-[#64748B]">Ins</div>
                    <div className="font-semibold text-[#0F172A]">{stats.insertions}</div>
                  </div>
                </div>
              </div>
            )}
